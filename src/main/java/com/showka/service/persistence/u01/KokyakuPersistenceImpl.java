package com.showka.service.persistence.u01;
...
import com.showka.service.persistence.u01.i.NyukinKakeInfoPersistence;
 ...
 */
@Service
public class KokyakuPersistenceImpl implements KokyakuPersistence {

	@Autowired
	private MKokyakuRepository repo;

	@Autowired
	private NyukinKakeInfoPersistence nyukinPersistence;

	@Autowired
	private BushoPersistence bushoService;
	...
	public void save(Kokyaku domain) {
		...
		if (domain.getHanbaiKubun() == HanbaiKubun.掛売) {
			...
			nyukinPersistence.deleteForciblyIfExists(domain.getRecordId());
		}
	}
	...
	public void delete(Kokyaku domain) {
		...
		// 入金掛情報delete
		nyukinPersistence.deleteForciblyIfExists(targetKokyaku.getRecordId());
		// 顧客を削除
